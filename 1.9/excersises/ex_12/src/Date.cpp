...
std::ostream& operator<<(std::ostream& os, const Month& m) {
	return os << Month_tbl[int(m)];						// prints month depending
}																						// depending on month number

Date::Date(long int dd)											// date constructor	for long int
	: days{dd} {
	if (!is_date(dd)) throw Invalid{};				// checks for validity
}
int Date::day() const {											// caches day
	if (!cache_valid)
		update_cache();
...
Month Date::month() const {									// caches month
	if (!cache_valid)
		update_cache();
...
int Date::year() const {										// caches year
	if (!cache_valid)
		update_cache();
...
void Date::update_cache() const {
	...
	// remove months
	while ((x = days_in_month(cache.year, cache.month)) && n > x) { // number of days in month
		n -= x;
		++cache.month;
	}
...
int days_in_month(int y, Month m) {	// number of days depending on month and year type
	switch (m) {
	case Month::feb:
